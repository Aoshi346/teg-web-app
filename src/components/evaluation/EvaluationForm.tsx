...
export default function EvaluationForm({ projectId, typeParam = 'proyecto', questions }: EvaluationFormProps) {
  ...
  const documentType = (typeParam || 'proyecto').toLowerCase() === 'tesis' ? 'Tesis' : 'Proyecto';
  const sourceQuestions = questions;
  const filteredQuestions = sourceQuestions.filter(q => !q.documentType || q.documentType === 'Both' || q.documentType === documentType);
