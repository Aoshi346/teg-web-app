...
const STATE_LABEL_TEG: Record<ProjectState, string> = {
  pending_review_1: "—",
  pending_review_2: "—",
  pending_defense: "—",
...
export default function TesisPage() {
  ...
  const tuAccion = counts.pending_articulo + counts.pending_entrega;
  const defensas = counts.pending_defensa;

  const tegOptions = useMemo<StateFilterOption[]>(
    () =>
      TEG_STATES.map((s) => ({
        state: s,
        label: STATE_LABEL_TEG[s],
        count: counts[s],
  ...
  const setStateFilter = (next: ProjectState | null) => {
    if (next == null) {
      router.push("/dashboard/tesis");
    } else {
      router.push(`/dashboard/tesis?state=${next}`);
    }
  };

  return (
    <>
      <DashboardHeader pageTitle="Tesis" />
        ...
        <div className="max-w-screen-2xl mx-auto">
          ...
          <section className="dashboard-hero-bg relative overflow-hidden rounded-2xl border border-border-subtle px-7 py-6 mb-6">
            <div className="relative z-10 flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
              <div className="flex flex-col gap-1.5">
                <p className="flex items-center gap-2 text-[10px] font-extrabold uppercase tracking-[0.14em] text-[var(--brand-orange)]">
                  <span aria-hidden className="inline-block h-[2px] w-6 rounded-full bg-gradient-to-r from-primary to-[var(--brand-orange)]" />
                  TEG · Período {selectedSemester}
                </p>
                ...
                <p className="max-w-[480px] text-sm font-medium leading-relaxed text-text-muted mt-1">
                  Registro y seguimiento de los trabajos de grado del período. El proceso comprende tres fases: artículo, entrega del ejemplar y defensa oral.
                </p>
              </div>
          ...
          {!isDataLoaded ? (
            ...
            <div className="bg-surface border border-dashed border-border-default rounded-2xl py-16 text-center">
              ...
              <h3 className="text-lg font-extrabold text-text-strong mb-1">No se encontraron tesis</h3>
