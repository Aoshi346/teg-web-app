...
| `src/app/globals.css` | Theme variables + custom animations + `pulse-soft` keyframe + `dashboard-hero-bg` utility (radial mesh + dot grid). + Seguimiento (Sub-G) classes (`.seg-table*`, `.tdot`, `.ladder*`, `.pgate*`, `.next-step`, `.seg-pill-summary`). |
